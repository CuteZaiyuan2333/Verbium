...
impl TabInstance for EmptyTab {
    ...
    fn ui(&mut self, ui: &mut Ui, _control: &mut Vec<AppCommand>) {
        ui.centered_and_justified(|ui| { ui.label("Verbium Layout Engine"); });
    }
...
impl Default for CorePlugin {
    fn default() -> Self { Self { new_file_counter: 1, show_about: false } }
}
...
impl Plugin for CorePlugin {
    ...
    fn on_tab_menu(&mut self, ui: &mut Ui, control: &mut Vec<AppCommand>) {
        ...
        ui.separator();
        if ui.button("Tile All").clicked() { control.push(AppCommand::TileAll); ui.close_menu(); }
        if ui.button("Reset Layout").clicked() { control.push(AppCommand::ResetLayout); ui.close_menu(); }
    }
    ...
    fn on_global_ui(&mut self, ctx: &egui::Context, _control: &mut Vec<AppCommand>) {
        egui::Window::new("About Verbium")
            ...
            .show(ctx, |ui| { ui.label("Verbium Framework v0.1.0"); });
    }
}